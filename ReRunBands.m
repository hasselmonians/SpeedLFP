    ...
    for i = 1:size(SL,1)
        ...
        for k = 1:4
            try
                ...
                [SL(i,k).bands.freq.meanSig, SL(i,k).bands.freq.sigmaSig,SL(i,k).bands.freq.speedDim,~,~,~,~,SL(i,k).bands.freq.intercepts, SL(i,k).bands.freq.tao] = Bands.speedVSfrequency(root,'ifPlot',0,'speedDim', (5:2.5:30)');
                [SL(i,k).bands.mag.meanSig, SL(i,k).bands.mag.sigmaSig,SL(i,k).bands.mag.speedDim,~,~,~,~,SL(i,k).bands.mag.intercepts, SL(i,k).bands.mag.tao] = Bands.speedVSmagnitude(root,'ifPlot',0,'speedDim', (5:2.5:30)');
                [SL(i,k).bands.pow.meanSig, ~, ~, ~, ~, ~, SL(i,k).bands.pow.intercepts, SL(i,k).bands.pow.tao] = Bands.speedVSpower(root,'ifPlot',0,'speedDim', (5:2.5:30)');
            end
        end
