...
meanSig = reshape(meanSig,size(roots,1),size(roots,2)); %mean frequency for each speed bin for each root object
...
[h_b p_b] = ttest(bsig(:,1),bsig(:,2),'tail','right');
[h_yint p_yint] = ttest(yintsig(:,1),yintsig(:,2),'tail','right');
